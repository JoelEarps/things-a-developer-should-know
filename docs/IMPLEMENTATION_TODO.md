...
- [ ] **Binary search**
  - Raw binary search implementation note in `algorithms/src/search_algorithms/binary_search.rs`.
...
- [ ] **Custom HashSet / HashMap**
  - `rust/.../hash_structures/hashsets/`: “Implementing my own hashset — what functions do we need?”
  - `rust/.../hash_structures/hashmaps/custom_hashmap.rs`: implement or add notes.

- [ ] **Pin**
  - `rust/src/async_rust/pin.rs`: complete TODO “Show this here”.

- [ ] **Copy vs Clone**
  - `rust/src/copy_vs_clone.rs`: try uncommenting the TODO lines and explain.

- [ ] **Performance optimisation in Rust**
  - `rust/src/performance_optimisation_in_rust/ReadMe.md`: item 5 — “Platform-Specific Implementations with cfg”.

- [ ] **Comparison sorting**
  - `practice/src/hackerrank/comparison_sorting.rs`: add proper type casting checks (replace panic/code smell).

- [ ] **Fixed-point maths**
  - `algorithms/src/fixed_point_and_floating_point/fixed_point_maths.rs`: generic/templated version (TODO in file).

- [ ] **Array ratio**
  - `practice/src/hackerrank/array_ratio.rs`: implement display for custom type with ToString.
...
- [ ] **Cache-friendly layout**
  - Add a small example (e.g. struct layout, iteration order) and optional benchmark; or document in `low-level/` or `rust/performance_optimisation_in_rust/`.
...
- [ ] **Trading bot risk at each stage**
  - Fill in `docs/trading-bot-risk-prep.md`: risks and checks for each stage (data, signal, pre-trade, order send, fill, ongoing); “if something goes wrong” section.

- [ ] **Pen-and-paper practice** (`docs/HFT-interview-prep.md` §21)
...
- [ ] **HFT prep doc — open items**
  - Add answers or notes for: “Garbage Collectors — what are they, why doesn’t Rust need them?”; “Stack vs Heap — when do you allocate, how?” (in `docs/HFT-interview-prep.md`).
...
- [ ] **One-hour test: Top K frequent elements**
  - `practice/src/hackerrank/one_hour_tests/general_comp_sci/top_k_frequent_elements.rs`: add clarifying questions, assumptions, example; implement solution.
...
| Trading bot risk   | `docs/trading-bot-risk-prep.md` |
| Sliding window     | `data_structures/src/sliding_window.rs` |
| Hash structures   | `rust/src/data_collections/arrays_and_hashing/hash_structures/` |
| Pin               | `rust/src/async_rust/pin.rs` |
